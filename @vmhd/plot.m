...
%     end
    ...
    % annotation(textbox)
    % title(obj.data.crit_sm);
    % title([obj.data.critthrcell]);
    % dim = [.2 .5 .3 .3];
    % annotation("textbox",dim,'String',obj.data.crit_sm(i),'FitBoxToText','on');
...
if(~isempty(Args.NumericArguments))
    ...
    crit_sm = obj.data.crit_sm(n);
    critthrcell = obj.data.critthrcell(n);
    comp = "";
    significance = false;
    if (crit_sm > critthrcell)
        comp = ">";
        significance = true;
    elseif (crit_sm < critthrcell)
        comp = "<";
    else
        comp = "=";
    end
    ...
    % dim = [.2 .5 .3 .3];
    % annotation("textbox",dim,'String',obj.data.crit_sm(n),'FitBoxToText','on');
    % c = colorbar(ax,'Position',[x y w h]);
else
	% plot all data
end
