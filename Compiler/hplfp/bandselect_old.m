...
%                     disp(q);
    ...
    subplot(2,3,4);
    thetalphaPowerSpec = reshape(thetaPower(1,:),[res,res])'; % reshape spikeFreq matrix to map onto physical maze locations
    imAlpha=ones(size(thetalphaPowerSpec)); % plot NaN regions as black
    imAlpha(isnan(thetalphaPowerSpec))=0;
    imagesc(thetalphaPowerSpec,'AlphaData',imAlpha); % input argument to set color range limits e.g. [100 200]
    thetaPValue = anova1(thetalphaPowerSpec,[],'off');
    ...
    title(strcat('SI: ',num2str(round(theta_locSI,2)),' p: ',num2str(thetaPValue)));
    ...
    imAlpha=ones(size(alphaPowerSpec)); % plot NaN regions as black
    imAlpha(isnan(alphaPowerSpec))=0;
    imagesc(alphaPowerSpec,'AlphaData',imAlpha); % input argument to set color range limits e.g. [100 200]
    ...
    title(strcat('SI: ',num2str(round(alpha_locSI,2)),' p: ',num2str(alphaPValue)));
    ...
    imAlpha=ones(size(gammaPowerSpec)); % plot NaN regions as black
    imAlpha(isnan(gammaPowerSpec))=0;
    imagesc(gammaPowerSpec,'AlphaData',imAlpha); % input argument to set color range limits e.g. [100 200]
    ...
    plot(2,4.5,'r.','MarkerSize',40);
    plot(4.5,4,'r.','MarkerSize',40);
    title(strcat('SI: ',num2str(round(gamma_locSI,2)),' p: ',num2str(gammaPValue)));
...
%     saveas(gcf,[name '.png']);
%     disp('Saved PNG')
    save([name '.mat'],'thetaTime','thetalphaPowerSpec','thetaPValue','alphaTime','alphaPowerSpec','alphaPValue','gammaTime','gammaPowerSpec','gammaPValue');
    disp('Saved MAT')
    close all;
