...
if nargin > 5
    ...
    if nargin > 6
        cellList = varargin(1);
        mapview = varargin{2};
        binDepths = varargin{3};
        fieldnum = varargin{4};
        peakcoords = varargin{5};
        cd(cellList{1});
    else
        cellList = varargin(1);
        cd(cellList{1});
            ...
            if plottype == 2 || plottype == 3 
                ...
                for kk = 1:2
                    ...
                    for mm = 1:obj.data.Args.GridSteps
                        mapGrid(mm,:) = mapLin( (mm-1)*obj.data.Args.GridSteps+1:mm*obj.data.Args.GridSteps );
                    end
...
elseif plottype == 5
    ...
    mapGrid = nan(obj.data.Args.GridSteps);
    for mm = 1:obj.data.Args.GridSteps
        mapGrid(mm,:) = mapLin( (mm-1)*gSteps+1:mm*gSteps );
    end
    
    % Load other place/sv object to compute SI
    switch objtype
        case 'place'
...
%     patch([peakcoords(1)-1 peakcoords(1)-2 peakcoords(1)+1 peakcoords(1)+1],[peakcoords(2)-1 peakcoords(2)+1 peakcoords(2)+1 peakcoords(2)-1], [0 0 0 0] ,[1 1 1 1],'FaceColor','none');
%     peakcoords;
    ...
    ax = gca;
%     set(ax,'XTickLabelMode','manual','XTickLabel',{},'YTickLabelMode','manual','YTickLabel',{});
%     ax.Title.String = horzcat('ch',num2str(identifiers(cell_ind,4)),'c',num2str(identifiers(cell_ind,5)),' SI=',num2str(obj2SI,2),'/',num2str(obj2shprc,2));
    set(ax,'Visible','Off');
    
    % For each pixel of place field, plit spatial view map
    pxSize = 0.3;
    pxcount = 1;
    for ii = 1:size(mapview,1)
        for jj = 1:size(mapview,2)
