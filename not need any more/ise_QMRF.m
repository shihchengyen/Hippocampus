...
%     bin_resolution = 0.01;
    ...
    for  i=1:size(combined,1)
        ...
        %The joint probability of every pixel
        hor_cond_entropy2 = hor_entropy2 - self_entropy;
        
        %   final calculation:
        mn = dim1*dim2;
        %The joint probability of every intensity
        ise_out1 = mn.*(vert_entropy + hor_cond_entropy);
        ise_out1 = ise_out1 - (mn/2).*(pos_angled_entropy + neg_angled_entropy);
        ise_out1 = ise_out1./mn;
        %The joint probability of every pixel
        ise_out2 = mn.*(vert_entropy2 + hor_cond_entropy2);
        ise_out2 = ise_out2 - (mn/2).*(pos_angled_entropy2 + neg_angled_entropy2);
    ...
    [ise_out]=ISE;
    disp(['time taken to calculate for ise_QMRF: ' num2str(toc)]);
end
