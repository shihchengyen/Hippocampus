    ...
    for di = 1:dlsize
        if(dlist(di).name(1)==Args.CalibFileNameChar)
            ...
            if(~isempty(s))
                ...
                sessionIndex = [];
                trialTimestamps = zeros(size(m,1), 3*noOfSessions); %stores timestamps at which start/cue/end trial events occured
				noOfTrials = []; %number of trials in each session
                ...
                for i=1:noOfSessions 
                    ...
                    if (exist('rplparallel.mat','file') == 0 )
                    % (not completed ) in case the rplparallel has not been created yet, try creating one with
                    % information from eyelink   
                    else
                         
                        rplObj = rplparallel('auto'); 
                        TrialNum =  size(rplObj.data.markers);
                        noOfTrials(i) = TrialNum(1); %get noOfTrials
                        nextSessionIndex = 3 * noOfTrials(i) + idx;  
                    ...
                    %stores the starting time of all the events into
                    %trialTimestamps
                    missingData=[];
                    correctedTimes = zeros(noOfTrials(i), 3);
                    sessionTimes = messageEvent(idx:idx+3*noOfTrials(i) -1);
                    
                    for f=1:size(sessionTimes,1)
                        r = floor(f/3) + 1; %row 
                        c = rem(f,3); %column
                        ...
                        correctedTimes(r,c)= sessionTimes(f);
                    end
                        
                    l = 1 + (i-1)*3; 
                    u = 3 + (i-1)*3;
                    row = noOfTrials(i);
                    trialTimestamps (1:row, l:u) = correctedTimes;     

                    %increase i value to go to next session folder
                    i=i+1;
                end
