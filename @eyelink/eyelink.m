...
if(dnum>0) %you can create the object now
    if (Args.Calibration) 
        ...
        type = cell2mat(type); 
        type = type - 24; %24 is the type for event messages generated by Unity 
    ...
    else %create the object for the Navigation Edf file  
        
        %get to the day directory and create the object
        fprintf('Creating eyelink object for Navigation session\n');
        str = getDataOrder('day');
        cd (str);
        dlist = nptDir ('.edf'); %look for the edf file in the session directory
