...
%   Example
%   
%   >> cd /Volumes/Hippocampus/Data/picasso/20180406/session01/array01
%   >> hmd = ProcessLevel(hidata,'Levels','Array','FileName','hmmsort.mat');
%   >> InspectGUI(hmd,'Array','UseGMR','Objects',{'vmswr',{'Trials',0,'Time',1,'Event',1}})
%
%   'Cmds': accepts cmds such as InspectGUI (to display each channel data
%   when analyzing array/session
%   e.g.
%   >> InspectGUI(hmd,'Objects',{{'vmswr',{'Cmds','InspectGUI(vmswr(''auto''),''addObjs'',...
%       {vmswr(''auto'')},''optArgs'',{{''PlotAllData''},{''Rms''}},''SP'',[2 1])'}}}

Args = struct('LabelsOff',0,'TitleOff', 0,'Color','b', ...
            'GroupPlots',1,'GroupPlotIndex',1,...
            'PreTrial',500,'Rms',0,'Cmds','',...
            'Trials',0,'Time',0,'Event',0,...
...
if(~isempty(Args.NumericArguments))
    ...
    text(di*sc, swrMean+swrThr*swrStd, strcat('threshold =',num2str(swrThr),'\sigma'));
    text(di*sc, swrMean+swrBeen*swrStd, strcat(num2str(swrBeen),'\sigma'));
    % p = gco;
    hold off
    ...
    if(Args.Trials) % plots # of SPW Event for each Trial
        if (Args.Time && ~Args.Event)
            for i = 1:length(tIdx)
                trialTime = tIdx(i,2)-tIdx(i,1);
                srwTime = 0;
                countIndex = (swrInfo(:,2)>=tIdx(i,1) & swrInfo(:,3)<=tIdx(i,2));
                swrTime = sum(swrInfo(countIndex,3)-swrInfo(countIndex,2));
                %edge cases
                startEdge = (swrInfo(:,2)<=tIdx(i,1) & swrInfo(:,3)>=tIdx(i,1));
                endEdge = (swrInfo(:,2)<=tIdx(i,3) & swrInfo(:,3)>=tIdx(i,3));
                if any(startEdge)
                    swrTime = swrTime + swrInfo(startEdge) - tIdx(i,1);
                end
                if any(endEdge)
                    swrTime = swrTime + tIdx(i,3) - swrInfo(endEdge);
                end
                tIdx(i,3) = swrTime./trialTime;
            end
        ...
        elseif (Args.Event && ~Args.Time)
            ...
            for i = 1:length(tIdx)
                ...
                tIdx(i,4) = swrTime./trialTime;
            end
            
            yyaxis left
            scatter(1:length(tIdx),tIdx(:,3))
            xlim([1 length(tIdx)])
    ...
    else % plots total SPW Event Time/Total Time
        if (Args.Time && ~Args.Event)
            swrTime = sum(swrInfo(:,3)-swrInfo(:,2));
            totalTime = max(tIdx(:,2))-min(tIdx(:,1));
            pcolor(0:1,0:1,swrTime./totalTime*[1 1;1 1])
            caxis([0 0.1])
            tx = 'Event Time/Total Time';
        elseif (Args.Event && ~Args.Time) % plots average # of SPW Event per Trial
            % count SPW Event in each trial 
            for i = 1:length(tIdx)
                tIdx(i,3) = sum(swrInfo(:,1)>tIdx(i,1) & swrInfo(:,1)<tIdx(i,2));
            end
            ...
            pcolor(0:1,0:1,swrTime./totalTime*[1 1;1 1])
            caxis([0 0.1])
            m = num2str(size(swrInfo,1));
            text(0.5,0.5,strcat('#:',m));
            tx = 'Event Time/Total Time';
        end
